...
#         self.__in_ds = h5py.File(file_path, "r")
#         self.__band_names = self.__in_ds.keys()
#
#     def get_array(self, band_name=None):
#         """
#         如果只有一个波段返回这个波段，否则根据名字来取波段
#         :param band_name: 需要提取的波段的名字，或者波段的序号，str，int
#         :return: 波段的矩阵
#         """
#         if band_name is None and len(self.__band_names) == 1:
#             return self.__in_ds[self.__band_names[0]][:]
#
#         if band_name in self.__band_names:
#             return self.__in_ds[band_name][:]
#         elif isinstance(band_name, int):
#             if band_name < len(self.__band_names):
#                 return self.__in_ds[self.__band_names[band_name]][:]
#
...
#         :return:
#         """
#         return self.get_array(band).shape()  # TODO 还未验证，需要重写
