...
class GdalBase(object):
    ...
    @staticmethod
    def layer_stacking_from_band_list(save_path, band_list):
        """从多个单波段栅格进行波段合成"""
        im_data = []
        no_data_value = []

        if len(band_list) >= 1:
            im_width, im_height, im_bands, im_geotrans, im_proj = GdalBase.read_tiff(band_list[0])[1:]
        else:
            raise EOFError('need at least one band')
...
class DealBigTiff(object):
    """用于处理大的 tiff 文件"""

    # 将大文件进行裁切，处理之后再进行合并

    def cut_tiff_to_block(self):
        """将 tiff 切割为 一个个的 block"""
        pass

    def merge_block(self):
        """将一个个的小块数据进行拼接，要处理好范围的问题，取左上角 block 和 右下角 block 得到经纬度范围即可"""
        pass
