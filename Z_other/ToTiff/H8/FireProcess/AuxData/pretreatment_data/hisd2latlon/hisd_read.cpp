...
int hisd_read_header(HisdHeader *header ,FILE *fp){
	...
	/* VNIR */
		...
		/* for version 1.3 */
		if((1>fread(&header->calib->coeff_mjd,8,1,fp)) ||
		(1>fread(&header->calib->gain_cnt2rad_mod,8,1,fp)) ||
		(1>fread(&header->calib->cnst_cnt2rad_mod,8,1,fp)) ||
		(1>fread(&header->calib->spareV,80,1,fp)))
			{
       			return(ERROR_READ);
			}
