...
def main():  # main()函数还负责创建了一些其他的全局常量，并且显示了在游戏运行的时候出现的初始屏幕。
    ...
    # showTextScreen('Tetromino')  # FIXME 设置在开始界面显示的文字
    ...
    while True:  # 游戏主循环，它负责砖块在落向底部的过程中，游戏主要部分的代码
        ...
        for event in pygame.event.get():  # 事件处理循环负责玩家旋转下落的砖块，移动下落的砖块。
            ...
            if event.type == KEYUP:  # 当按键弹起的时候响应KEYUP事件
                ...
                elif (event.key == K_RIGHT):  # 同上  # FIXME pygame.locals 这些都是 pygame 中的关键字
                    movingRight = False
        ...
        DISPLAYSURF.fill(BGCOLOR)  # FIXME 绘制背景
        drawBoard(board)  # FIXME 绘制砖块
        drawStatus(score, level)  # FIXME 绘制已经存在的砖块
        drawNextPiece(nextPiece)  # FIXME 绘制下一个砖块
